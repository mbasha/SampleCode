...
function user_exist_check ($username, $password, $email, $firstname, $lastname){
  $query = "select * from members where username='$username'";
	...
	if (!$result){
		...
		if($row == 0){
			//$query = "insert into members values (NULL, '$username','$password','$email','$firstname','$lastname', NULL, NULL)";
      $query = "insert into playlists values (NULL, 'Favorites', '$username')";
			...
			$query = "insert into members (user_id, username, password, email, firstname, lastname, fav_list, profilepic)
                  values (NULL, '$username','$password','$email','$firstname','$lastname',
                            (select playlist_id from playlists where username='$username'), NULL)";
...
function uploadMessage($sender, $receiver, $message){
	$query = "insert into messages values (NULL,'$sender','$receiver','$message',0)";
//        echo  "insert query:" . $query;
        $result = mysql_query( $query );
	if($result)
		return 1;
